...
2. Create a new SDM project on the [Projects](https://console.nest.google.com/device-access/project-list) page
   1. Give your project a name so it is easily recognizable
   2. "Skip" entering the OAuth client ID for now
   3. If you want to download camera images using the binding, it is required to "Enable" events.
...
3. Copy and save the **Project ID** at the top of the page (e.g. `585de72e-968c-435c-b16a-31d1d3f76833`) somewhere
...
2. Create a new client on the "Credentials" page ([APIs & Services > Credentials](https://console.cloud.google.com/apis/credentials)):
   1. Click the "Create Credentials" button at the top of the page
   2. Choose "OAuth client ID"
   3. As "Application type" choose "Web application"
   4. Give it a name so you can remember what it is used for (e.g. `Nest Binding SDM`)
   5. Add "https://www.google.com" to the "Authorized redirect URIs"
   6. Click "Create" to create the client
   7. Copy and save the generated **Client ID** (e.g. `1046297811237-3f5sj4ccfubit0fum027ral82jgffsd1.apps.googleusercontent.com`) and **Client Secret** (e.g. `726kcU-d1W4RXxEJA79oZ0oG`) somewhere
3. Configure the SDM project to use the created client:
   1. Go the the SDM [Projects](https://console.nest.google.com/device-access/project-list) page
   2. Click on your SDM Project to show its details
   3. Scroll to "Project Info > OAuth client ID" and open the options menu (3 stacked dots) at the end of the line
   4. Select the "Edit" option
   5. Copy/paste the saved OAuth 2.0 Client ID here (e.g. `1046297811237-3f5sj4ccfubit0fum027ral82jgffsd1.apps.googleusercontent.com`)
   6. Click the "Save" button at the end of the line to update the project
...
2. Copy/paste the saved SDM **Project ID** to SDM group parameter in the SDM Account Thing configuration parameters (e.g. `585de72e-968c-435c-b16a-31d1d3f76833`)
3. Copy/paste the saved OAuth 2.0 **Client ID** to SDM group parameter (e.g. `1046297811237-3f5sj4ccfubit0fum027ral82jgffsd1.apps.googleusercontent.com`)
4. Copy/paste the saved OAuth 2.0 **Client Secret** to SDM group parameter (e.g. `726kcU-d1W4RXxEJA79oZ0oG`)
5. Create an authorization code for the binding:
   ...
   2. Enable all the permissions you want to use with the binding and click "Next" to continue

   3. Login using your Google account when prompted

   4. On the "Google hasn't verified this app" page, click on "Advanced"

   5. Then click on "Go to ... (advanced)"

   6. Now "Allow" the SDM permissions and confirm your choices again by clicking "Allow"

   7. After your browser has been redirected to <https://www.google.com>, the **Authorization Code** will be set in the browser URL as value of the "code" URL query parameter

   8. Copy/paste the **Authorization Code** to the SDM group parameter in the openHAB Nest SDM Account Thing configuration

6. All required SDM Account Thing configuration parameters have now been entered so create it by clicking "Create Thing"
...
2. Scroll to "Project Info > Pub/Sub topic" and check if it is set to "Enabled"
3. If it is set to "Disabled", enable events:
   1. Open the options menu (3 stacked dots) at the end of the line
   2. Select the "Edit" option
   3. Check the "Enable events" option
   4. Click the "Save" button at the end of the line to update the project
...
2. Copy and save the GCP **Project ID** (e.g. `openhab-12345`)
...
4. As "Application type" choose "Web application"
5. Give it a name so you can remember what it is used for (e.g. `Nest Binding Pub/Sub`)
6. Add "https://www.google.com" to the "Authorized redirect URIs"
7. Click "Create" to create the client
8. Copy and save the generated **Client ID** (e.g. `1046297811237-lg27h26kln6r1nbg54jpg6nfjg6h4b3n.apps.googleusercontent.com`) and **Client Secret** (e.g. `1-k78-XcHhp_gdZF-I6JaIHp`) somewhere
...
2. Copy/paste the saved GCP **Project ID** to Pub/Sub group parameter (e.g. `openhab-123`)
3. Enter a name in **Subscription ID** that uniquely identifies the Pub/Sub subscription used by the binding
...
4. Copy/paste the saved OAuth 2.0 **Client ID** to Pub/Sub group parameter (e.g. `1046297811237-lg27h26kln6r1nbg54jpg6nfjg6h4b3n.apps.googleusercontent.com`)
5. Copy/paste the saved OAuth 2.0 **Client Secret** to Pub/Sub group parameter (e.g. `1-k78-XcHhp_gdZF-I6JaIHp`)
6. Create an authorization code for the binding:
   ...
   2. Login using your Google account when prompted

   3. On the "Google hasn't verified this app" page, click on "Advanced"

   4. Then click on "Go to ... (advanced)"

   5. Now "Allow" the Pub/Sub permissions and confirm your choices again by clicking "Allow"

   6. After your browser has been redirected to <https://www.google.com>, the **Authorization Code** will be set in the browser URL as value of the "code" URL query parameter

   7. Copy/paste the **Authorization Code** to the Pub/Sub group parameter in the openHAB Nest SDM Account Thing configuration

7. All required Pub/Sub Account Thing configuration parameters have now been entered so click "Save" to update the SDM Account Thing configuration
