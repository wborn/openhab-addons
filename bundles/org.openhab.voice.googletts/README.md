...
5. Go to the `APIs & Services` -> `Credentials` page in the GCP Console and your project ([link](https://console.cloud.google.com/apis/credentials))
6. From the `Create credentials` drop-down list, select `OAuth client ID`.
7. Select application type `Web application` and enter a name into the `Name` (e.g. `OpenHAB TTS`) field.
8. Add `https://www.google.com` to the `Authorized redirect URIs`.
